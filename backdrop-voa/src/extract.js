...
const data=await p.evaluate(()=>{
 ...
 const sel='.welcome .k, .welcome h1, .lbl, .cap, .cell .txt, .foot .l, .foot .p';
 const texts=[...document.querySelectorAll(sel)].map(e=>{const cs=getComputedStyle(e);
   return {text:e.innerText,cls:e.className||e.tagName,...r(e),fs:parseFloat(cs.fontSize),fw:cs.fontWeight,color:cs.color,ls:parseFloat(cs.letterSpacing)||0,align:cs.textAlign}});
 return {imgs,texts}});
require('fs').writeFileSync('layout.json',JSON.stringify(data,null,1));
await p.addStyleTag({content:`
 .welcome .k,.welcome h1,.lbl,.cap,.cell .txt,.foot .l,.foot .p{visibility:hidden}
 img{opacity:0}
