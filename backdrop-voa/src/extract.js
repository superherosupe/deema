...
const data=await p.evaluate(()=>{
 ...
 const sel='.welcome .k, .welcome h1, .lbl, .cap, .tag, .st b, .st span, .mcap, .mi em, .mi b, .mi span:not(.th), .cell .txt, .foot .l, .foot .p';
...
await p.addStyleTag({content:`
 .welcome .k,.welcome h1,.lbl,.cap,.tag,.st b,.st span,.mcap,.mi b,.mi span:not(.th),.cell .txt,.foot .l,.foot .p{visibility:hidden}
 img{opacity:0} .mi em{color:transparent!important}
